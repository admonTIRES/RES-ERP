ID_FORMULARIO_INVENTARIO_MANTENIMIENTO = 0
...
var inventario_id_principal = null; 
...
$("#guardarINVENTARIO").click(function (e) {
    ...
    if (ID_FORMULARIO_INVENTARIO_MANTENIMIENTO == 0) {
        ...
        alertMensajeConfirm({
            ...
            await ajaxAwaitFormData({ api: 1, ID_FORMULARIO_INVENTARIO_MANTENIMIENTO: ID_FORMULARIO_INVENTARIO_MANTENIMIENTO }, 'InventarioSaveMantenimiento', 'formularioINVENTARIO', 'guardarINVENTARIO', { callbackAfter: true, callbackBefore: true }, () => {
                ...
                $('.swal2-popup').addClass('ld ld-breath')
                    ...
                    inventario_id = data.inventario.ID_FORMULARIO_INVENTARIO_MANTENIMIENTO;
                    ID_FORMULARIO_INVENTARIO_MANTENIMIENTO = data.inventario.ID_FORMULARIO_INVENTARIO_MANTENIMIENTO
                    alertMensaje('success','Información guardada correctamente', 'Esta información esta lista para usarse',null,null, 1500)
                    //  $('#Modal_inventario').modal('hide')
                    // document.getElementById('formularioINVENTARIO').reset();
                    Tablainventariomantenimiento.ajax.reload()
                    cargarMetricas();
...
var Tablainventariomantenimiento = $("#Tablainventariomantenimiento").DataTable({
    ...
    ajax: {
        dataType: 'json',
        method: 'GET',
        url: '/Tablainventariomantenimiento',
        beforeSend: function () {
            mostrarCarga();
        },
        complete: function () {
            Tablainventariomantenimiento.columns.adjust().draw();
            ocultarCarga();
        },
    ...
    columns: [
        ...
        { data: 'ID_FORMULARIO_INVENTARIO_MANTENIMIENTO' },
    ...
    drawCallback: function () {
        ...
        const table = document.querySelector('#Tablainventariomantenimiento');
...
$.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
    ...
    if (settings.nTable.id !== 'Tablainventariomantenimiento') {
        return true;
    }

    if (!colorSeleccionado) return true;

    let row = Tablainventariomantenimiento.row(dataIndex).node();
...
function cargarMetricas() {
    ...
    $.ajax({
        url: '/metricasInventariomantenimiento',
        type: 'GET',
...
$('#Tablainventariomantenimiento tbody').on('click', 'td>button.EDITAR', function () {
    var tr = $(this).closest('tr');
    var row = Tablainventariomantenimiento.row(tr);
    
    ID_FORMULARIO_INVENTARIO_MANTENIMIENTO = row.data().ID_FORMULARIO_INVENTARIO_MANTENIMIENTO;

    inventario_id = row.data().ID_FORMULARIO_INVENTARIO_MANTENIMIENTO;

    inventario_id_principal = row.data().INVENTARIO_ALMACEN_ID;
    ...
    $("#tab2-entrada").off("click").on("click", function () {
        cargarTablaentradainventariomantenimiento();
    });
    ...
    if (row.data().FOTO_EQUIPO) {
        ...
        var imagenUrl = '/mostrarFotoEquipoMantenimiento/' + row.data().ID_FORMULARIO_INVENTARIO_MANTENIMIENTO + extension;
   ...
   if (row.data().REQUIERE_ARTICULO === "1") {
       ...
       cargarDocumentos(inventario_id_principal);
...
$(document).ready(function() {
    $('#Tablainventariomantenimiento tbody').on('click', 'td>button.VISUALIZAR', function () {
        var tr = $(this).closest('tr');
        var row = Tablainventariomantenimiento.row(tr);
...
$('#Tablainventariomantenimiento tbody').on('change', 'td>label>input.ELIMINAR', function () {
    var tr = $(this).closest('tr');
    var row = Tablainventariomantenimiento.row(tr);
    ...
    eliminarDatoTabla(data, [Tablainventariomantenimiento], 'inventarioDeleteMantenimiento');
});
...
function cargarTablaentradainventariomantenimiento() {
    if ($.fn.DataTable.isDataTable('#Tablaentradainventariomantenimiento')) {
        Tablaentradainventariomantenimiento.clear().destroy();
    }

    Tablaentradainventariomantenimiento = $("#Tablaentradainventariomantenimiento").DataTable({
        ...
        ajax: {
            ...
            method: 'GET',
            cache: false,
            url: '/Tablaentradainventariomantenimiento',
            beforeSend: function () {
                $('#loadingIcon').css('display', 'inline-block');
            },
            complete: function () {
                $('#loadingIcon').css('display', 'none');
                Tablaentradainventariomantenimiento.columns.adjust().draw();
            },
            error: function (jqXHR, textStatus, errorThrown) {
                $('#loadingIcon').css('display', 'none');
                alertErrorAJAX(jqXHR, textStatus, errorThrown);
...
function cargarTablaDocumentosEquipo() {
    ...
    Tabladocumentosinventario = $("#Tabladocumentosinventario").DataTable({
        ...
        ajax: {
            dataType: 'json',
            data: { equipo: inventario_id_principal }, 
            method: 'GET',
            cache: false,
...
function cargarDocumentos(inventario_id_principal) {
    $.ajax({
        url: `/obtenerDocumentosPorInventario/${inventario_id_principal}`,
        type: 'GET',
        dataType: 'json',
...
function guardarDetallearticulo() {
    ...
    fetch('/guardarDetallearticulo', {
        ...
        body: JSON.stringify({
            ID_FORMULARIO_INVENTARIO_MANTENIMIENTO: inventario_id,
    ...
    .then(data => {
        ...
        if (data.code === 1) {
            Tablainventariomantenimiento.ajax.reload(null, false);
        } else {
